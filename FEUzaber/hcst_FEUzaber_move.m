function resPos = hcst_FEUzaber_move(bench,pos)
%hcst_FEUzaber_move Function to move the FEUzaber to an absolute position (in mm)
%   
...
%
%   Arguments/Outputs:
%   resPos = hcst_FEUzaber_move(bench, pos) moves the FEUzaber to the position
%       specified by 'pos'. 
...
%
%   Examples:
%       hcst_FEUzaber_move(bench, [8.8])
%           Moves the vertical axis to 8.8 and the horizontal axis to 45
%
%       hcst_FEUzaber_move(bench, [NaN 45])
%           Move only the horizontal axis (to 45)
%
%
%   See also: hcst_setUpBench, hcst_FEUzaber_getPos, hcst_cleanUpBench
%
...
if pos > bench.FEUzaber.BOUND
    pos = bench.FEUzaber.BOUND;
end
...
if ~isnan(pos(1))
    ...
    try
        bench.FEUzaber.axis.moveAbsolute(pos, Units.LENGTH_CENTIMETRES);
    catch exception
...
try
    resPos = bench.FEUzaber.axis.getPosition(Units.LENGTH_CENTIMETRES)*M2MM;
catch exception
    rethrow(exception);
end
