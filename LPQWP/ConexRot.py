...
class Conex_Device(object):
    ...
    MVTIMEOUT   = 1000       #(MVTIMEOUTxDELAY)= number of seconds device will 
    ...
    def open(self):
        ...
        *Does not reopen device if already open
        ...
        if self.ser.isOpen():
            print('(SN:%s) is already open' %self.SN)
        else:
        ...
        if self.ser.isOpen():
            ...
            if (self.isMoving(rd) or self.isHoming(rd)):#rd[-2:] in ('28', '1E'):
                ...
                msgOut = '  Device Type      : ' + self.TY + ''\
                       ...
                       '\n  Software Min Lim.: ' + str(self.MNPS) + ''\
            ...
            msgOut = '  Device Type      : ' + self.TY + ''\
                   ...
                   '\n  Software Min Lim.: ' + str(self.MNPS) + ''\
                   '\n  Software Max Lim.: ' + str(self.MXPS) + '\n' + msgOut + ''\
                   '\n  Deadband         : ' + rd[5][3:].strip().decode() + ''\
                   '\n  Home Search Type : ' + rd[8][3:].strip().decode() + ''\
                   '\n  Interpol. Factor : ' + rd[6][3:].strip().decode() + ''\
                   '\n  Integral Gain    : ' + rd[3][3:].strip().decode() + ''\
                   '\n  Proportional Gain: ' + rd[4][3:].strip().decode() + ''\
                   '\n  L-P Filter  Freq.: ' + rd[2][3:].strip().decode() + ''\
                   '\n  RS-485 Address   : ' + rd[9][3:].strip().decode() + ''\
    ...
    def enable(self):
        ...
        if erFlg:
            print('DEV ERROR::: Device returned error:\n' +
                  '  ' + self.errorStr(erCd))
            if erCd == 'H':
                print('ERROR::: Device likely not homed:\n' +
                      '  solution: call home()')
            return False
    ...
    def errorStr(self, erCd):
        ...
        #Check if port is open
        if not self.ser.isOpen():
            print('WARNING::: Device must be open to translate string\n' + 
                  '  solution: call open()')
            return erCd
    ...
    def reqLim(self):
        ...
        *To simply display the values, use devPrint()
        ...
        self.MNPS   = -2147000000 #float(rd[3:]) 
        ...
        self.MXPS  = 2147000000 #float(rd[3:])
