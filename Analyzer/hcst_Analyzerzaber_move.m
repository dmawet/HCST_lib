function resPos = hcst_Analyzerzaber_move(bench,pos)
%hcst_Analyzerzaber_move Function to move the Analyzerzaber to an absolute position (in mm)
%   
...
%
%   Arguments/Outputs:
%   resPos = hcst_Analyzerzaber_move(bench, pos) moves the Analyzerzaber to the position
%       specified by 'pos'. 
...
%
%   Examples:
%       hcst_Analyzerzaber_move(bench, [8.8])
%           Moves the vertical axis to 8.8 and the horizontal axis to 45
%
%       hcst_Analyzerzaber_move(bench, [NaN 45])
%           Move only the horizontal axis (to 45)
%
%
%   See also: hcst_setUpBench, hcst_Analyzerzaber_getPos, hcst_cleanUpBench
%
...
if pos > bench.Analyzerzaber.BOUND
    pos = bench.Analyzerzaber.BOUND;
end
...
if ~isnan(pos(1))
    ...
    try
        bench.Analyzerzaber.axis.moveAbsolute(pos, Units.LENGTH_CENTIMETRES);
    catch exception
...
try
    resPos = bench.Analyzerzaber.axis.getPosition(Units.LENGTH_CENTIMETRES)*M2MM;
catch exception
    rethrow(exception);
end
bench.Analyzerzaber.curr_pos = pos;
end
