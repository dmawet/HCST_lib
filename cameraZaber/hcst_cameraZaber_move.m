function resPos = hcst_cameraZaber_move(bench,pos)
%hcst_cameraZaber_move Function to move the cameraZaber to an absolute position (in mm)
%   
...
%
%   Arguments/Outputs:
%   resPos = hcst_cameraZaber_move(bench, pos) moves the cameraZaber to the position
%       specified by 'pos'. 
...
%
%   Examples:
%       hcst_cameraZaber_move(bench, [8.8])
%           Moves the vertical axis to 8.8 and the horizontal axis to 45
%
%       hcst_cameraZaber_move(bench, [NaN 45])
%           Move only the horizontal axis (to 45)
%
%
%   See also: hcst_setUpBench, hcst_cameraZaber_getPos, hcst_cleanUpBench
%
...
if pos > bench.cameraZaber.BOUND
    pos = bench.cameraZaber.BOUND;
end
...
if ~isnan(pos(1))
    ...
    try
        bench.cameraZaber.axis.moveAbsolute(pos, Units.LENGTH_CENTIMETRES);
    catch exception
...
try
    resPos = bench.cameraZaber.axis.getPosition(Units.LENGTH_CENTIMETRES)*M2MM;
catch exception
    rethrow(exception);
end
