function hcst_setUpCameraZaber(bench)
...
disp('*** Setting up cameraZaber stage... ***');
% import zaber.motion.binary.*;
% import zaber.motion.*;
import zaber.motion.ascii.*;
import zaber.motion.*;
try
    conn = Connection.openSerialPort('/dev/ttyAndorZaber');
...
bench.cameraZaber.conn = conn;
bench.cameraZaber.axis = axis;

bench.cameraZaber.CONNECTED = true;
% 
disp('*** Zaber stages for cameraZaber initialized. ***');
% 
