...
disp('*** Setting up LS stage... ***');
% import zaber.motion.binary.*;
% import zaber.motion.*;
import zaber.motion.ascii.*;
import zaber.motion.*;
try
    conn = Connection.openSerialPort('/dev/ttyLSZaberH');
...
bench.LS.H.conn = conn;
bench.LS.H.axis = axis;

bench.LS.H.CONNECTED = true;
% 
disp('*** Zaber stages for LS initialized. ***');
% 
