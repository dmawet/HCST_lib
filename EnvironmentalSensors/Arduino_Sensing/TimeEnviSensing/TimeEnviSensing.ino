...
int MINUTE = 20;
int SECOND = 0;
int NUMDAYSARR[] = {31,28,31,30,31,30,31,31,30,31,30,31};
int DAYSINMON = NUMDAYSARR[MONTH-1];
...
// NTP Servers:
//
//IPAddress timeServer(132, 163, 4, 101); //from Github example
...
time_t getNtpTime() {
  ...
  //Grady Note - code is returning 0, resets time
  //changed to 
  return 50000; // return 0 if unable to get the time
}
...
void getFileName(char *filename, time_t t) {
  snprintf(filename, 13, "%04d%02d%02d.csv", YEAR, MONTH, DAY);
  //snprintf(filename, 13, "%04d%02d%02d.csv", 0000, 0, 02);
  //snprintf(filename, 13, "GradyCadenceTest.csv");
}
...
void writeData(Print &f) {
  ...
  snprintf(data, 10, "%02d:%02d:%02d", HOUR, MINUTE, SECOND);
  f.write(data);
  for (uint8_t i = 0; i < NUMPORTS; i++) {
    f.write(",");
...
void takeMeasurements() {
  ...
  if (!file.open(filename, O_WRITE | O_APPEND)) {
    ...
    file.write(filename);
    file.write("</a></td>\n<td>");
    snprintf(filename, 11, "%04d-%02d-%02d", YEAR, MONTH, DAY);
    file.write(filename);
    file.write("</td>\n</tr>\n</table>\n</body>\n</html>");
    file.close();
    getFileName(filename, global_t);
  ...
  if (second(global_t) == 0 and minute(global_t) % 2 == 0) {
    writeData(file);
    MINUTE = MINUTE + 2;
    if (SECOND >= 60) {
      SECOND = 0;
